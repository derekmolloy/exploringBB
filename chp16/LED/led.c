...
static int __init ebb_LED_init(void){
   ...
   sprintf(ledName, "led%d", gpioLED);      // Create the gpio name for /sys/ebb/led60
   ...
   // add the attributes to /sys/ebb/ -- for example, /sys/ebb/led60/ledOn
   ...
   ledOn = true;
   gpio_request(gpioLED, "sysfs");          // gpioLED is 60 by default, request it
